...
## 支持请求和和数据返回的拦截 ##
依然是上面的核心流程代码，在设置好请求参数后，作者定义了一个**chain数组**，同时放入了dispatchRequest, undefined这两个元素对应promise的resolve和reject方法，之后将请求拦截器的成功和失败处理依次压入**chain数组头部**，将返回拦截器的成功和失败处理依次推入**chain数组尾部**。
