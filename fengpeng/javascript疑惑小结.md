函数是JavaScript中非常重要的一部分，通过函数来封装一些语句，完成特定的功能，在适当的地方可以多次使用，使得代码逻辑更加清晰。多人开发中，使用现有的函数（方法），可以专注于业务逻辑，提升开发效率
###如何定义函数
1、函数声明
...
**定义函数的小建议**
- 函数名称不要太随意了，比如用'a','b'等等。要根据函数的功能设置一个能表达函数功能的标识符，当函数名称是多个单词的组合时，建议使用驼峰命名法
- 添加注释，使得阅读和使用更加清晰。我们可以使用jsDoc提供的注释规则来为函数添加注释，使用jsDoc规范之后可以更加方便的通过工具来生成API文档。
```
/**
 * @description 获取搜索结果
 * @params {Object} params - 必填项，以json对象形式传参
 * @params {String} params.search  - 必填项, 搜索关键字
 * @return {Promise}
 * */
...
```
通过注释，我们可以很清楚了了解到函数的功能，参数应该怎么传递，以及函数的返回结果是什么。在多人合作开发中，别人很清楚该如何调用已经定义的函数。更多与jsDoc相关的内容，可以参考github上的开源项目[JsDoc](https://github.com/jsdoc3/jsdoc)
### 函数如何调用
